...
#    self.wfile.write(bytes("<p>You accessed path: %s</p>" % self.path, "utf-8"))
      ...
      self.wfile.write(self._unknown)
      print("unknown")
  ...
  def do_POST(self):
    ...
    print (str(content_length) +' '+ post_data) # <-- Print post data
    self._set_headers()
    self.wfile.write("<html><body><h1>POST!</h1></body></html>")
    gpio24 = GPIO.LOW
    gpio26 = GPIO.LOW
    if content_length > 0:
      m = re.search('green=on', post_data)
      if m:
        gpio24 = GPIO.HIGH
      m = re.search('red=on', post_data)
      if m:
        gpio26 = GPIO.HIGH 
